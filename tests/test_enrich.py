...
def test_dataclass_transform(dash_duo, args, kwargs):
    ...
    # Check that stuff works. It doesn't using a normal Dash object.
    dash_duo.start_server(app)
    time.sleep(0.5)  # wait for callback code to execute.
...
def test_dataclass_transform_list(dash_duo):
    ...
    @app.callback(Output("log", "children"), Input("store", "data"))
    def update_log(states: list[StateModel]):
        return r" - ".join(
            f"{state.value.isoformat()}: {state.list_of_values}" for state in states
        )
    ...
    assert dash_duo.find_element("#log").text == '2000-01-01T00:00:00: [1, 2, 3, 4, 5] - 2000-01-02T00:00:00: [6, 7, 8, 9, 10]'
